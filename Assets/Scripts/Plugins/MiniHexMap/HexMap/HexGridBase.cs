...
namespace MiniHexMap
{
    public abstract class HexGridBase : MonoBehaviour
    {
        ...
        {
            return Resources.Load<GameObject>("Prefabs/House/Tribe_" + index);
        }

        protected GameObject GetFieldPrefab(int index = 0)
        {
            return Resources.Load<GameObject>("Prefabs/House/Field_" + index);
        }

        protected GameObject GetWallPrefab()
        {
            return Resources.Load<GameObject>("Prefabs/House/Wall");
        }
        ...
        public GameObject SetPrefabAtTop(GameObject prefab, HexCell cell, float scale = 1f, bool rotate = true)
        {
        ...
        public void SetGameObjectAtTop(GameObject go, HexCell cell)
        {
            Vector3 position = cell.transform.position;
            position.y = Mathf.Max(cell.Elevation, 1) * HexMetrics.elevationStep;
            go.transform.position = position;
